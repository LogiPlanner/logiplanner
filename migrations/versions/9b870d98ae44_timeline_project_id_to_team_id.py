...
def upgrade() -> None:
    ...
    # Step 3: Validate that every row was backfilled before enforcing NOT NULL
    connection = op.get_bind()
    missing_team_id_count = connection.execute(
        sa.text("""
            SELECT COUNT(*)
    ...
    if missing_team_id_count:
        raise RuntimeError(
            "Cannot migrate timeline_entries.project_id to team_id: "
            f"{missing_team_id_count} row(s) could not be backfilled. "
            "Repair or remove timeline_entries rows with missing/invalid project references "
            "before rerunning this migration."
        )

    # Step 4: Make team_id NOT NULL now that it's populated
    op.alter_column('timeline_entries', 'team_id', nullable=False)

    # Step 5: Add FK constraint to teams
    ...
    # Step 6: Create index for faster team-scoped queries
    op.create_index('ix_timeline_entries_team_id', 'timeline_entries', ['team_id'])

    # Step 7: Drop the old project_id FK and column
